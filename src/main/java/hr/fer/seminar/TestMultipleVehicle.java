...
public class TestMultipleVehicle {

	private static int NUMBER_OF_TESTS = 30;
	...
	public static void main(String[] args) {
		...
		try (Scanner sc = new Scanner(System.in)) {
			...
			if(input == 1) {
				...
				double sum = 0;
				int minimumNumberOfVehicles = Integer.MAX_VALUE;
				for(int i = 0; i < NUMBER_OF_TESTS; i++) {
					System.out.println("Iteration: " + i);
					Genotype<ProgramGene<Double>> programDynamic = gp.calculate();
					int numberOfVehicles = gp.getUsedVehicles(programDynamic).size();
					sum += numberOfVehicles;
					System.out.println("Number of vehicles: "  + numberOfVehicles);
					if(numberOfVehicles < minimumNumberOfVehicles) {
						minimumNumberOfVehicles = numberOfVehicles;
					}
				}
				
				System.out.println("Minimum number of vehicles: " + minimumNumberOfVehicles);
				System.out.println("Average number of vehicles: " + sum/NUMBER_OF_TESTS);
				...
				for(int i = 0; i < NUMBER_OF_TESTS; i++) {
					...
					int numberOfVehicles = ga.getUsedVehicles(programDynamic).size();
					sum += numberOfVehicles;
					System.out.println("Number of vehicles: "  + numberOfVehicles);
					if(numberOfVehicles < minimumNumberOfVehicles) {
						minimumNumberOfVehicles = numberOfVehicles;
